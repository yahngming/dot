...
arch() {
	sudo pacman -S 7zip aerc android-tools atool base-devel bat bc bluez bluez-utils brightnessctl btop cmus cups dante eza fastfetch fd ffmpegthumbnailer fontforge fuse fzf git git-delta glow inetutils jq less lf lolcat man-db mpv nsxiv openssh pass perl-image-exiftool pipewire polkit poppler python-pywal rclone ripgrep rpi-imager samba timeshift translate-shell udiskie ueberzugpp unrar unzip w3m wireplumber zathura-pdf-poppler zoxide # base
	sudo pacman -S noto-fonts noto-fonts-cjk noto-fonts-emoji otf-firamono-nerd ttf-nerd-fonts-symbols # font
	...
	yay -S bibata-cursor-git chicago95-theme ctpv-git dragon-drop google-chrome mihomo newsraft r-quick-share-bin terminaltexteffects-git
