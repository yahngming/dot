...
arch() {
	sudo pacman -Syu aerc atool base-devel bat bc bluez bluez-utils btop clash cmus dante eza fastfetch fcitx5-chinese-addons fcitx5-im fd ffmpegthumbnailer firefox fontforge fzf git git-delta glow gvim jq less lf libreoffice-fresh lolcat maim man-db mpv noto-fonts noto-fonts-cjk noto-fonts-emoji nsxiv openssh pass perl-image-exiftool polkit poppler python-pywal samba steam translate-shell trash-cli ttf-nerd-fonts-symbols ueberzugpp w3m xcape xclip xcompmgr xdo xorg-xrandr xssstate xwallpaper zathura-pdf-poppler zoxide
	mkdir -p ~/.local/src
