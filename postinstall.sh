...
arch() {
	...
	sudo pacman -S --noconfirm 7zip aerc atool base-devel bat bc bluetui bluez-utils brightnessctl btop chafa cmus cups dante docker docker-compose evtest eza f3d fastfetch fd ffmpegthumbnailer fontforge fortune-mod fuse fzf git git-delta glow id3v2 imagemagick impala jq lazydocker less lf lolcat man-db mpv ollama openssh pass perl-image-exiftool polkit poppler python-pywal rclone ripgrep smbclient translate-shell udiskie unrar unzip w3m wiremix wireplumber yt-dlp zathura-pdf-poppler zoxide # base
	...
	yay -S --noconfirm bibata-cursor-git chicago95-theme dragon-drop newsraft python-pywalfox r-quick-share sing-box #base
